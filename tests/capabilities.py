...
class DatabaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.BLOBBinary = self.db_module.Binary(
            (u"".join([unichr(i) for i in range(256)] * 16)).encode("latin1")
        )
        client_hooks.install_all_patch()
    ...
    def tearDown(self):
        if self.leak_test:
            ...
            self.failIf(
                orphans,
                "%d orphaned objects found after deleting cursor" % orphans,
            )
            ...
            self.failIf(
                orphans,
                "%d orphaned objects found after deleting connection"
                % orphans,
            )
    ...
    def test_transactions(self):
        ...
        self.cursor.execute(
            "select col1 from %s where col1=%s" % (self.table, 0)
        )
        l = self.cursor.fetchall()
    ...
    def test_CHAR(self):
        ...
        self.check_data_integrity(
            ("col1 char(255)", "col2 char(255)"), generator
        )
